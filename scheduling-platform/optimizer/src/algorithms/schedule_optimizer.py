...
class UtilizationOptimizer:
    def __init__(self, api_key: str = None):
        # If API key is not provided, try to use the one provided in the parameter
        if api_key is None:
            # If still None, use the hardcoded key (the one provided by you)
            api_key = "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"
            print("Using hardcoded Claude API key")
...
Include only the CSV content in your response, no explanation or other text."""
    ...
    def optimize(self):
        ...
        # Define output files to use from previous pipeline stages
        output_files = {
            ...
            for key, filename in output_files.items():
                ...
                if file_path.exists():
                    ...
                    try:
                        ...
                        # Mapping keys to our consistent internal naming
                        key_mapping = {
                            'master_schedule': 'current_master_schedule',
                            'student_assignments': 'current_student_assignments',
                            'teacher_schedule': 'current_teacher_assignments',  # Rename to match existing code
                            'utilization_report': 'current_utilization_report'
                        }
                        
                        # Use the proper mapped key name
                        data[key_mapping.get(key, f"current_{key}")] = df
