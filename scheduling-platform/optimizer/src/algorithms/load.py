...
class ScheduleDataLoader:
    ...
    def __init__(self, input_dir=None):
        ...
        # Allow setting the input_dir directly rather than hard-coding
        self.input_dir = Path(input_dir) if input_dir else self.project_root / 'input'
        
        # Create debug directory within the input directory if possible
        if self.input_dir.exists():
            self.debug_dir = self.input_dir / 'debug'
        else:
            # Fallback to project_root/debug
        ...
        self.summary_file = self.debug_dir / f"data_loader_{timestamp}.log"
